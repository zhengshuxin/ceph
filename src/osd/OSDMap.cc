                                   ...
                                   vector<int> *acting, int *acting_primary) const
{
  ...
  vector<int> *_acting = (acting ? acting : new vector<int>);
  int *_up_primary = (up_primary ? up_primary : new int);
  int *_acting_primary = (acting_primary ? acting_primary : new int);
  _pg_to_osds(*pool, pg, &raw, _up_primary);
  _raw_to_up_osds(pg, raw, _up, _up_primary);
  _get_temp_osds(*pool, pg, _acting, _acting_primary);
  if (acting && _acting->empty())
    *_acting = *_up;
  if (acting_primary && *_acting_primary == -1)
  ...
  if (_acting != acting)
    delete _acting;
  if (_acting_primary != acting_primary)
    delete _acting_primary;
}
